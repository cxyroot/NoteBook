...
redis.conf配置文件，改完后确保生效，记得重启，记得重启
   ...
   3 默认 bind 127.0.0.1             改为  直接注释掉(默认bind 127.0.0.1只能本机访问)或改成本机IP地址，否则影响远程IP连接
...
## redis设置外网访问
    ...
    /redis-server  /home/soft/myredis/redis.conf
    
    redis-cli
    
    [root@192 bin]# ./redis-server /home/soft/myredis/redis.conf 
    39759:C 07 Jan 2024 22:14:47.197 # WARNING Memory overcommit must be enabled! Without it, a background save or replication may fail under low memory condition. Being disabled, it can also cause failures without low memory condition, see https://github.com/jemalloc/jemalloc/issues/1328. To fix this issue add 'vm.overcommit_memory = 1' to /etc/sysctl.conf and then reboot or run the command 'sysctl vm.overcommit_memory=1' for this to take effect.
...
#### 2.他什么时候fork子进程，或者什么时候触发rdb持久化机制
 ...
 配置文件中默认的快照配置
 执行命令save或者bgsave  save是只管保存，其他不管，全部阻塞  

 bgsave： redis会在后台异步进行快照操作，同时可以响应客户端的请求
 执行flushall命令  但是里面是空的，无意义

```
save 手动执行会死锁 使得redis不在执行其他任务
bgsave 不会阻塞   bgsave： redis会在后台异步进行快照操作，同时可以响应客户端的请求

redis-check -rdb /home/soft/myredis/dump6379.rdb


禁用RDB
在命令行中执行 临时操作：redis-cli config set save “”
...
<img src="C:\Users\Sunray\AppData\Roaming\Typora\typora-user-images\image-20240109220615119.png" alt="image-20240109220615119" style="zoom:50%;" />



![image-20240109221637465](C:\Users\Sunray\AppData\Roaming\Typora\typora-user-images\image-20240109221637465.png)
